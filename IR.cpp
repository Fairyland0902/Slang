...
static llvm::Value* calcArrayIndex(shared_ptr<AST_ArrayIndex> index, GenCodeContext &context)
{
    ...
    {
        auto temp = make_shared<AST_BinaryOperator>(make_shared<AST_Integer>(sizeVec[i]), TIMES, index->expressions->at(i-1));
        expression = make_shared<AST_BinaryOperator>(temp, PLUS, expression);
    }
