...
public:
    ...
    shared_ptr<AST_Identifier> getSymbolType(string name) const
    {
        for (auto it = blockStack.rbegin(); it != blockStack.rend(); it++)
        {
    ...
    void setSymbolType(string name, shared_ptr<AST_Identifier> value)
    {
    ...
    {
        CodeGenBlock *generateCodeBlock = new CodeGenBlock();
        generateCodeBlock->block = block;
        generateCodeBlock->returnValue = nullptr;
        blockStack.push_back(generateCodeBlock);
    }

    void popBlock()
    {
        CodeGenBlock *generateCodeBlock = blockStack.back();
        blockStack.pop_back();
        delete generateCodeBlock;
    }
